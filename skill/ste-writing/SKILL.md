---
name: ste-writing
description: Rewrite prose (docs, READMEs, PR descriptions, error messages, release notes, comments — never code) into ASD-STE100 Simplified Technical English to remove "AI slop". Use when asked to make writing not sound like AI, make docs clear or plain, enforce a controlled writing style, or write technical documentation that reads human. Two modes — strict (procedures/safety) and STE-flavored (general prose). Verifies the result with the ste-lint command before returning it.
---
...
## Verify with the linter (required before returning)

Run the machine-checkable subset against your output. This is the step that turns the skill from advice into a checked claim.

1. Save the rewritten text to a temporary file, for example `/tmp/ste-draft.md`.
2. Run `ste-lint /tmp/ste-draft.md` from a bash tool.
3. Read the score: violations per 100 words (`per100w`). Lower is cleaner. The goal is 0; anything below ~2 for long prose is acceptable.
4. The output lists every violation category and, for banned/marketing words, samples of the offending words. Fix each flagged issue in the draft: split overlong sentences, replace semicolons and contractions, make passives active, swap banned words, drop marketing adjectives and hedges.
5. Re-run `ste-lint /tmp/ste-draft.md` until clean (or as low as the text realistically allows). Do not return text that still trips the lintable rules.
6. Return the final text. If the user will benefit, report the before/after `per100w` score — the delta is the signal.

If `ste-lint` is not found, say so and ask the user to run the install script (README: `./install.sh`), then lint again. Do not skip the check.

## Self-lint (run before you lint with the tool)
